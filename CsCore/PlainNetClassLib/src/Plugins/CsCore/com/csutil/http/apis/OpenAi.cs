...
namespace com.csutil.http.apis {
    ...
    public class OpenAi {
        ...
        public Task<CompletionsResponse> Complete(string prompt) {
            return Complete(new CompletionsRequest() { prompt = prompt });
        }

        public Task<CompletionsResponse> Complete(CompletionsRequest requestParams) {
            var request = new Uri("https://api.openai.com/v1/completions").SendPOST();
            return request.WithAuthorization(apiKey).WithJsonContent(requestParams).GetResult<CompletionsResponse>();
        }
